...
class User_model extends CI_Model
{
    ...
    {
        $this->db->select("fname,lname,var_email,var_mobile_no,company,c_logo,gistin,bisno,catAs");

        $this->db->from("mst_users");
        $where = "(var_state='$var_state' AND var_city='$var_city') AND (catAs='$cat' OR regAs='$cat')";
        $this->db->where($where);
        $query = $this->db->get();
    ...
    public function updateUser($userId)
    {
        ...
        $this->common_model->updateRow("mst_users", $data, [
            "int_glcode" => $userId,
        ]);
